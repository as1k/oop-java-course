...
//		if(new File("C:\\Users\\User\\eclipse-workspace\\sub-workspace\\Intranet\\src\\Intranet\\userlist").exists()) {Database.desUsers();}
//		Database.update();
		Database.users.add(new TechSupportGuy("Nurik", "Nurikovich", "tech", "tech", 2500000));
		Database.users.add(new Admin("Aseke", "Nurikovich", "admin", "admin", 19));
			...
			if (auth == null) {
				...
				menu: while(true) {
					if (auth instanceof Admin) {
						...
						switch(choice) {
						case 1: System.out.print("Student's name/surname/login/password/speciality/yearOfStudy: ");
					...
					if (auth instanceof Manager) {
						...
						case 1: ((Manager) auth).showOpenCourses();
								...
								break;
						case 2: ((Manager) auth).viewStudentandTeacher(); break;
						case 6: save(); break menu;
						}
						
					}
					if (auth instanceof TechSupportGuy) {
						((TechSupportGuy)auth).getMenu();
						int newChoice = sc.nextInt();
						switch(newChoice) {
						case 1: ((TechSupportGuy)auth).viewNewOrders(); break;
						case 2: ((TechSupportGuy)auth).viewDone(); break;
						case 3: ((TechSupportGuy)auth).viewAccButNotDone(); break;
						case 6: save(); break menu;
						}
					}
				}
